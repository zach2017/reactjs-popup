...
export default ({
  ...
  const [isModal, setModal] = useState(modal ? true : !trigger);
  ...
  const onMouseEnter = () => {
    ...
    timeOut = setTimeout(() => openPopup(), mouseEnterDelay);
  };
  const onMouseLeave = () => {
    clearTimeout(timeOut);
    timeOut = setTimeout(() => closePopup(), mouseLeaveDelay);
  };

  const setPosition = () => {
    if (isModal) return;
    const helper = helperEl.current.getBoundingClientRect();
    const trigger = triggerEl.current.getBoundingClientRect();
    const content = contentEl.current.getBoundingClientRect();
    const boundingBox = getTooltipBoundary(keepTooltipInside);
    ...
    contentEl.current.style.top = `${cords.top - helper.top}px`;
    contentEl.current.style.left = `${cords.left - helper.left}px`;
    if (arrow) {
      arrowEl.current.style.transform = cords.transform;
      arrowEl.current.style['-ms-transform'] = cords.transform;
      arrowEl.current.style['-webkit-transform'] = cords.transform;
      arrowEl.current.style.top = arrowStyle.top || cords.arrowTop;
      arrowEl.current.style.left = arrowStyle.left || cords.arrowLeft;
    }

    if (
      window
        .getComputedStyle(triggerEl.current, null)
        .getPropertyValue('position') === 'static' ||
      window
        .getComputedStyle(triggerEl.current, null)
        .getPropertyValue('position') === ''
    )
      triggerEl.current.style.position = 'relative';
  };

  const addWarperAction = () => {
    // const { contentStyle, className, on } = props;
    // const { modal } = state;
  ...
  // on EScape && repositionOnResize  effect

  useEffect(() => {
    if (closeOnEscape) window.addEventListener('keyup', onEscape);
    if (repositionOnResize)
      window.addEventListener('resize', repositionOnResizeFn);

    return () => {
      if (closeOnEscape) window.removeEventListener('keyup', onEscape);
      if (repositionOnResize)
        window.removeEventListener('resize', repositionOnResizeFn);
    };
  }, [repositionOnResize, closeOnEscape]);

  // content render
  const renderTrigger = () => {
    const triggerProps = {key: 'T', ref: triggerEl};
    // const { on, trigger } = props;
